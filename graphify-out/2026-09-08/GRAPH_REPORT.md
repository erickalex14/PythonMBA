...
## Summary
- 976 nodes · 1688 edges · 70 communities (58 shown, 12 thin omitted)
- Extraction: 96% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 60 edges (avg confidence: 0.63)
...
Cohesion: 0.19
Nodes (9): CATEGORY_PALETTE, ParetoChart(), ScatterXY(), sliceTreemap(), smoothPath(), Treemap(), TREEMAP_SHADES, TreemapItem (+1 more)
...
Cohesion: 0.05
Nodes (66): AtsPage(), poppins, EstadisticasVentasPage(), poppins, Celda, MetasKpiPage(), periodoActual(), poppins (+58 more)
...
### Community 11 - "get"
Cohesion: 0.18
Nodes (11): download_ats(), download_estadisticas_ventas(), download_liquidaciones(), download_movimientos(), download_ventas(), get, Descarga el reporte de Ventas Espejo en formato Excel para el rango dado., Descarga el reporte de Ventas por Producto (Estadísticas de Inventario) en Excel (+3 more)
...
Cohesion: 0.23
Nodes (10): Inyección de Dependencia de FastAPI para validar el API Key de Next.js.     Aseg, verify_api_key(), get_ats_service(), get_estadisticas_service(), get_excel_service(), Provee el Servicio de ATS inyectando su dependencia del Repositorio., Provee el Servicio de Excel., Provee el Servicio de Estadisticas de Ventas por producto. (+2 more)

### Community 15 - "LiquidacionesCharts.tsx"
Cohesion: 0.20
Nodes (6): RankedBarChart(), EMPRESA_COLOR, EMPRESA_COLOR_FALLBACK, LiquidacionesCharts(), LiquidacionesChartsProps, num()
...
Cohesion: 0.14
Nodes (11): get, Obtiene la lista de movimientos de productos filtrados por rango de fechas., read_movimientos(), get_movimientos_service(), Provee el Servicio de Movimientos inyectando su dependencia del Repositorio., Config, MovimientoDTO, BaseModel (+3 more)
...
Cohesion: 0.24
Nodes (6): poppins, Card(), CardProps, CardVariant, Modal(), ModalProps
...
Cohesion: 0.06
Nodes (41): get, Dashboard de ventas en una sola llamada: totales por rango (hoy, ayer,     sema, Totales del rango con devoluciones desglosadas (con devoluciones, solo     devo, Resumen agregado (hoy/ayer/semana/mes/año calendario + producto más     vendido, Obtiene la lista de transacciones del reporte de Ventas Espejo.     Requiere va, read_dashboard_ventas(), read_resumen_ventas(), read_totales_ventas() (+33 more)
...
### Community 44 - "reporte_conciliacion_proveedores.py"
Cohesion: 0.33
Nodes (8): consultar(), login(), main(), norm(), Reporte: facturas recibidas conciliadas con transferencias bancarias. Cruce PROV, Normaliza llaves de cruce: quita .0 de floats, espacios, mayusculas., Deriva tipo de contribuyente del RUC ecuatoriano (3er digito).     ponytail: heu, tipo_contribuyente()

### Community 45 - "app/main.py"
Cohesion: 0.06
Nodes (48): Sincroniza Movimientos/Liquidaciones/ATS/Ventas con los datos de AYER.     Corre, run_nightly_sync(), start_scheduler(), stop_scheduler(), authenticate_docs(), get_open_api_endpoint(), get_swagger_documentation(), lifespan() (+40 more)
...
Cohesion: 0.28
Nodes (5): ExpandableChartCard(), EstadisticasVentasCharts(), fmtMoney(), fmtNumber(), Props
...
Cohesion: 0.25
Nodes (3): get_db(), Busca en INVT_Ficha_Principal los campos de "ultima venta" del ERP. Hipotesis: U, Explora estructura+datos de tablas ERP (grupo PROVEEDORES) para diseñar reporte
...
Cohesion: 0.16
Nodes (10): ABC, Mba3Repository, procesar_respuesta_erp(), Parser robusto para interceptar respuestas de error o ausencia de registros, Implementación concreta del repositorio utilizando la librería 'requests'     pa, Lista los ORIGIN_MEMO reales (sin truncar) del kardex del ERP en un rango, para, Check del reintento ante HTTP 401 en Mba3Repository.ejecutar_consulta. Sin red:, test_no_reintenta_infinito_si_sigue_401() (+2 more)

### Community 64 - "_resolver_env"
Cohesion: 0.43
Nodes (6): Normaliza el entorno del ERP.      Antes, un valor no reconocido caia a `setting, _resolver_env(), Un entorno de ERP mal escrito no debe terminar consultando PRODUCCION.  Antes, `, test_desconocido_falla(), test_por_defecto(), test_validos()

### Community 65 - "CustomExportRequest"
Cohesion: 0.40
Nodes (5): CustomExportRequest, export_custom_data(), BaseModel, post, Exporta una lista personalizada de objetos JSON (datos filtrados del cliente) a

### Community 66 - "TrendLineAdvanced.tsx"
Cohesion: 0.50
Nodes (4): ChartTooltip(), TrendLineAdvanced(), TrendLineAdvancedProps, useMeasuredWidth()

### Community 71 - "AtsCharts.tsx"
Cohesion: 0.13
Nodes (16): AtsCharts(), AtsChartsProps, IVA_COLOR, num(), RadialGauge(), TierHeading(), TwoBarComparison(), DevolucionesDonut() (+8 more)
...
## Knowledge Gaps
- **161 isolated node(s):** `TotalesEmpresa`, `KPICardsProps`, `RangoVentas`, `TopProducto`, `TopsPorVista` (+156 more)
...
- **What is the exact relationship between `Frontend Logo (logo.svg)` and `Python MBA Wordmark / Brand Identity`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **Why does `IMba3Repository` connect `IMba3Repository` to `sync_controller.py`, `kpi_controller.py`, `VentasService`, `app/main.py`, `dependencies.py`, `EstadisticasVentasService`, `AtsService`, `MovimientosService`, `importar_excel`, `Mba3Repository`?**
...
- **Why does `ExcelService` connect `ExcelService` to `CustomExportRequest`, `kpi_controller.py`, `get`, `dependencies.py`, `get`?**
  _High betweenness centrality (0.029) - this node is a cross-community bridge._
- **Are the 8 inferred relationships involving `IMba3Repository` (e.g. with `BodegaIn` and `MetaIn`) actually correct?**
  _`IMba3Repository` has 8 INFERRED edges - model-reasoned connections that need verification._
