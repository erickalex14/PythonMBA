# Graph Report - Python MBA  (2026-09-12)

## Corpus Check
- 144 files · ~100,175 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1022 nodes · 1773 edges · 76 communities (63 shown, 13 thin omitted)
- Extraction: 96% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 72 edges (avg confidence: 0.61)
...
9. `LiquidacionesService` - 17 edges
10. `authOptions` - 17 edges
...
Cohesion: 0.21
Nodes (14): ComparisonMiniCard(), DailySalesDashboard(), DailySalesDashboardProps, dateNDaysAgo(), daysBefore(), deltaPct(), fetchRange(), fetchVentasAdaptive() (+6 more)
...
Cohesion: 0.14
Nodes (18): _clean(), ExcelService, DataFrame, Servicio de Utilidad para la Generación de Archivos Excel.     Todos los report, 2 hojas: "Consolidado" con lo que ya se sabe que es cada columna, y         "Va, Fallback genérico (sin encabezado/resumen/totales) para datos ad-hoc que no, Replica el reporte nativo "Estadisticas de Inventarios" del ERP: hoja principal, Escribe el reporte ENCIMA del ultimo archivo que subio Contabilidad. (+10 more)

### Community 13 - "LiquidacionesService"
Cohesion: 0.09
Nodes (22): get, Obtiene el consolidado de liquidaciones y detalles de productos importados., read_liquidaciones(), Config, LiquidacionDTO, BaseModel, codigo_sin_sufijo_empresa(), LiquidacionesService (+14 more)

### Community 14 - "dependencies.py"
Cohesion: 0.18
Nodes (14): Inyección de Dependencia de FastAPI para validar el API Key de Next.js.     Aseg, verify_api_key(), get_ats_service(), get_estadisticas_service(), get_excel_service(), get_liquidaciones_service(), get_ventas_service(), Provee el Servicio de Liquidaciones inyectando su dependencia del Repositorio. (+6 more)

### Community 15 - "LiquidacionesCharts.tsx"
Cohesion: 0.16
Nodes (9): TwoBarComparison(), EMPRESA_COLOR, EMPRESA_COLOR_FALLBACK, LiquidacionesCharts(), LiquidacionesChartsProps, num(), EMPRESA_LABELS, getEmpresaCode() (+1 more)
...
Cohesion: 0.15
Nodes (6): get_mba3_repository(), get_sync_service(), Provee la implementación por defecto del Repositorio de MBA3., Provee el Servicio de Sincronización inyectando su dependencia del Repositorio., IMba3Repository, Interface para el Repositorio de Datos de MBA3 ERP.
...
### Community 28 - "admin_controller.py"
Cohesion: 0.27
...
Cohesion: 0.11
Nodes (12): Dias con datos y dias sin datos (huecos) por tipo de sincronizacion., SyncService, DbFalso, _Query, Check: con el ERP caido, ejecutar_consulta(estricto=True) devuelve None (no []),, Registra si alguien llamo .delete() sobre el staging., Simula el ERP inalcanzable: token OK, consultas None., RepoFalsoCaido (+4 more)
...
Cohesion: 0.13
Nodes (16): AtsCharts(), AtsChartsProps, IVA_COLOR, num(), RadialGauge(), RankedBarChart(), TierHeading(), DevolucionesDonut() (+8 more)

### Community 72 - "scripts"
Cohesion: 0.40
Nodes (5): scripts, build, dev, lint, start

### Community 73 - "MovimientosService"
Cohesion: 0.16
...
- **What is the exact relationship between `Frontend Logo (logo.svg)` and `Python MBA Wordmark / Brand Identity`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **Why does `IMba3Repository` connect `IMba3Repository` to `sync_service.py`, `MovimientosService`, `kpi_controller.py`, `VentasService`, `SyncService`, `LiquidacionesService`, `dependencies.py`, `Mba3Repository`, `AtsService`, `EstadisticasVentasService`, `MovimientoStaging`, `importar_excel`?**
  _High betweenness centrality (0.062) - this node is a cross-community bridge._
- **Why does `SyncService` connect `SyncService` to `sync_controller.py`, `app/main.py`, `sync_service.py`, `AtsFacturaStaging`, `VentasFacturaStaging`, `dependencies.py`, `IMba3Repository`, `MovimientoStaging`?**
  _High betweenness centrality (0.033) - this node is a cross-community bridge._
- **Why does `ExcelService` connect `ExcelService` to `kpi_controller.py`, `CustomExportRequest`, `LiquidacionesService`, `dependencies.py`, `get`?**
...
- **Are the 5 inferred relationships involving `ExcelService` (e.g. with `CustomExportRequest` and `BodegaIn`) actually correct?**
  _`ExcelService` has 5 INFERRED edges - model-reasoned connections that need verification._